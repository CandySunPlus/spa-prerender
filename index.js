...
function SPAPrerender(staticDir, apps, options) {
    ...
    this.routes = [].concat.apply(
        [],
        apps.map(app => app.routes.map(route => ({ entry: app.entry, path: route })))
    );
    ...
    return Promise.all(
        self.routes.map(function(route) {
            ...
            return new Promise(function(resolve, reject) {
                compileToHTML(self.staticDir, route.entry, route.path, self.options, function(
                    prerenderedHTML
                ) {
                    ...
                    mkdirp(folder, function(error) {
                        ...
                        FS.writeFile(file, prerenderedHTML, function(error) {
                            if (error) {
